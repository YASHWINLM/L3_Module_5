...
public class LongChipCompetition {
	...
	public void Longest() {
		this.initializeBeatles();
		double isLongest=0;
		String Winner="";
		for (int i = 0; i < theBeatles.size(); i++) {
			for (int j = 0; j < theBeatles.get(i).getChips().size(); j++) {
				if (	theBeatles.get(i).getChips().get(j).getLength()>isLongest) {
					isLongest=	theBeatles.get(i).getChips().get(j).getLength();
					Winner=	theBeatles.get(i).getName();
				}
			
			}
		}
		System.out.println(Winner+" Had the longest chip it was "+isLongest+" inches long");
	}
